...
Additions to `App` (`crates/zenmon-tui/src/app.rs`):
...
`ConnectMode` is already exported from `zenmon-core::config`. `zenmon-tui` already depends on `zenmon-core` via `ZenmonConfig`, so no new crate boundary is crossed.

`App::new` is updated to accept the initial mode (or to read it from a passed-in `ZenmonConfig`) so `current_mode` is initialized correctly. The CLI entry point (`crates/zenmon-cli/src/main.rs`) wires the resolved `cfg.mode` through.

### Reconnect plumbing

In the main loop (`crates/zenmon-tui/src/lib.rs`), add a block adjacent to the existing `pending_reconnect_port` handler:
...
Unit tests in `crates/zenmon-tui/src/app.rs` `#[cfg(test)]` block:
