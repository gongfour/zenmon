...
├── crates/
│   ├── zemon-core/        # Zenoh 연결, 디스커버리, 구독, 쿼리 로직
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
...
│   ├── zemon-cli/         # clap 기반 서브커맨드 (headless CLI)
│   │   ├── Cargo.toml
│   │   └── src/main.rs
│   └── zemon-tui/         # ratatui TUI 대시보드
│       ├── Cargo.toml
│       └── src/main.rs
```
...
- **zemon-core** — Zenoh 세션 관리, 토픽 디스커버리, 구독 스트림, 쿼리, 노드 레지스트리 조회. CLI와 TUI 모두 이 라이브러리에 의존.
- **zemon-cli** — `clap` 서브커맨드. stdout으로 JSON/텍스트 출력. 스크립팅/파이프라인 친화적. `zemon tui` 서브커맨드로 TUI 실행. zemon-tui를 라이브러리로 의존.
- **zemon-tui** — ratatui 기반 인터랙티브 TUI. 라이브러리 크레이트 (별도 바이너리 아님). zemon-cli에서 호출.

바이너리는 `zemon` 하나 (zemon-cli가 생성). `zemon tui`로 TUI 모드 진입.

## CLI 서브커맨드

```
zemon <COMMAND> [OPTIONS]
...
```
zemon discover [KEY_EXPR]
  # 활성 키 목록 출력. KEY_EXPR 생략 시 "**"
  # 예: zemon discover "forklift/**"

zemon sub <KEY_EXPR> [--pretty] [--timestamp]
  # 실시간 메시지 스트림 출력. Ctrl+C로 종료.
  # 예: zemon sub "forklift/snapshot" --pretty

zemon query <KEY_EXPR> [--payload <JSON>] [--timeout <MS>]
  # Zenoh GET 요청 후 응답 출력
  # 예: zemon query "forklift/status"

zemon nodes [--watch]
  # 등록된 노드 목록 출력
  # --watch: 변경 시 실시간 업데이트

zemon tui [--refresh <MS>]
...
| 설정 | CLI 플래그 | 환경변수 | 기본값 |
|------|-----------|---------|--------|
| 엔드포인트 | `--endpoint` | `ZEMON_ENDPOINT` | `tcp/localhost:7447` |
| 모드 | `--mode` | `ZEMON_MODE` | `client` |
| namespace | `--namespace` | `ZEMON_NAMESPACE` | (없음) |
| 설정파일 | `--config` | `ZEMON_CONFIG` | (없음) |
...
- zemon-core: 세션 관리, 디스커버리, 구독, 쿼리
- zemon-cli: discover, sub, query, nodes 서브커맨드
- zemon-tui: 5개 뷰 (Dashboard, Topics, Subscribe, Query, Nodes)
