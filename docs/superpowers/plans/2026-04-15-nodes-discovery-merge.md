...
**Goal:** Make the zenmon TUI Nodes view reflect the real Zenoh network by merging router admin-space data (live, 2s polling) with multicast scout results (manual trigger), with per-source tracking and scout-staleness visualization.

**Architecture:** Two independent background tokio tasks in the TUI produce `Vec<NodeInfo>` — one polling `@/*/router` admin space every 2 seconds, one running multicast `scout` on demand. A pure `merge_nodes` function combines both sides in `zenmon-core`; results flow into `App` via new `AppEvent` variants and are rendered in a 4-column Nodes view with source badges and stale dimming.
...
**Worktree assumption:** The user will create an isolated git worktree + branch before executing this plan. All paths below are relative to the worktree root, which mirrors `D:\project\hdx\zenmon`. If you start on `master`, create a branch first:
...
**Created:**
- `crates/zenmon-core/src/merge.rs` — pure merge function + unit tests

**Modified:**
- `Cargo.toml` — add `bitflags` to `[workspace.dependencies]`
- `crates/zenmon-core/Cargo.toml` — depend on workspace `bitflags`
- `crates/zenmon-core/src/lib.rs` — register new `merge` module
- `crates/zenmon-core/src/types.rs` — add `NodeSources` bitflags, modify `NodeInfo` fields, add `is_scout_stale()` + unit tests
- `crates/zenmon-core/src/registry.rs` — rename `list_nodes` → `query_admin_nodes`, implement `@/*/router` JSON parsing with `sessions[]` expansion
- `crates/zenmon-core/src/scout.rs` — add `ScoutInfo::to_node_info()` helper
- `crates/zenmon-cli/src/main.rs` — update two call sites from `list_nodes` → `query_admin_nodes`
- `crates/zenmon-tui/src/event.rs` — add `AdminNodes` / `ScoutStarted` / `ScoutNodes` variants; store `tx` in `EventHandler`; expose `sender()`
- `crates/zenmon-tui/src/app.rs` — add `admin_nodes`, `scout_nodes`, `scout_in_progress`, `last_scout_at`, `pending_scout_request`; add event handlers; remove `node_selected` range bugs; wire `s` key binding for Nodes view
- `crates/zenmon-tui/src/lib.rs` — remove old `list_nodes` call paths, spawn admin polling task + scout trigger helper, use `events.sender()`
- `crates/zenmon-tui/src/views/nodes.rs` — 4-column layout with Source column, footer counts, stale dimming
...
**Files:**
- Modify: `Cargo.toml`
- Modify: `crates/zenmon-core/Cargo.toml`
...
zenmon-core = { path = "crates/zenmon-core" }
zenmon-tui = { path = "crates/zenmon-tui" }
...
- [ ] **Step 2: Add bitflags to zenmon-core**

Edit `crates/zenmon-core/Cargo.toml`. Add one line:
...
Run: `cargo check -p zenmon-core`
Expected: PASS with no errors. (May emit warnings — ignore.)

- [ ] **Step 4: Commit**

```bash
git add Cargo.toml crates/zenmon-core/Cargo.toml
...
**Files:**
- Modify: `crates/zenmon-core/src/types.rs:64-72`
- Test: same file (module-inline `#[cfg(test)] mod tests`)

- [ ] **Step 1: Replace the existing `NodeInfo` struct and add `NodeSources`**

In `crates/zenmon-core/src/types.rs`, replace lines 64-72 (the existing `NodeInfo` block) with:
...
Append to the same file `crates/zenmon-core/src/types.rs`:
...
Run: `cargo test -p zenmon-core types::tests`
...
**Files:**
- Modify: `crates/zenmon-core/src/registry.rs` (full replacement)

- [ ] **Step 1: Replace the entire contents of `crates/zenmon-core/src/registry.rs`**
...
- [ ] **Step 2: Verify zenmon-core now builds**
...
```bash
git add crates/zenmon-core/src/registry.rs
...
**Files:**
- Modify: `crates/zenmon-cli/src/main.rs:167` and `:193`

- [ ] **Step 1: Confirm the call sites**

Run: `grep -n "list_nodes" crates/zenmon-cli/src/main.rs`
Expected output:
```
167:            let nodes = zenmon_core::registry::list_nodes(&session).await?;
193:                            let updated = zenmon_core::registry::list_nodes(&session).await?;
```

- [ ] **Step 2: Rename both call sites**

In `crates/zenmon-cli/src/main.rs`, replace both occurrences:

- Line 167: `list_nodes(&session)` → `query_admin_nodes(&session)`
- Line 193: `list_nodes(&session)` → `query_admin_nodes(&session)`

Use find-and-replace for `zenmon_core::registry::list_nodes` → `zenmon_core::registry::query_admin_nodes`.

- [ ] **Step 3: Build the CLI**

Run: `cargo check -p zenmon-cli`
Expected: PASS. If the compile error mentions `NodeInfo::last_seen`, the CLI is reading that field somewhere — search with `grep -n "last_seen" crates/zenmon-cli/src/main.rs`. Remove any such references (the CLI nodes printer at lines 170-184 only uses `.zid`, `.kind`, `.locators`, so should be clean).

- [ ] **Step 4: Commit**

```bash
git add crates/zenmon-cli/src/main.rs
...
**Files:**
- Create: `crates/zenmon-core/src/merge.rs`
- Modify: `crates/zenmon-core/src/lib.rs` (register module)
...
Edit `crates/zenmon-core/src/lib.rs`. Add `pub mod merge;` after `pub mod registry;`:
...
Run: `cargo test -p zenmon-core merge::tests`
Expected: all four tests PASS.

- [ ] **Step 4: Commit**

```bash
git add crates/zenmon-core/src/merge.rs crates/zenmon-core/src/lib.rs
...
**Files:**
- Modify: `crates/zenmon-core/src/types.rs` (append below existing `ScoutInfo`)

- [ ] **Step 1: Add the conversion method**

In `crates/zenmon-core/src/types.rs`, immediately after the `pub struct ScoutInfo { ... }` block, add:
...
Edit `crates/zenmon-tui/src/event.rs`. Replace the existing enum at lines 7-12 with:
...
pub enum AppEvent {
    ...
    Zenoh(ZenohMessage),
    Tick,
    AdminNodes(Vec<zenmon_core::types::NodeInfo>),
    ScoutStarted,
    ScoutNodes(Vec<zenmon_core::types::NodeInfo>),
}
```
...
The new variants use `zenmon_core::types::NodeInfo`. The file already has `use zenmon_core::types::ZenohMessage;` at the top. Change that line to:
...
```bash
git add crates/zenmon-tui/src/event.rs
...
In `crates/zenmon-tui/src/app.rs` line 4, replace:
...
use zenmon_core::types::{NodeInfo, NodeSources, TopicInfo, ZenohMessage};
use std::time::{Duration, SystemTime};
```
...
In `crates/zenmon-tui/src/app.rs`, change line 59 from:
...
Run: `grep -n "Char('s')\|Char('S')" crates/zenmon-tui/src/app.rs`
...
At the top of `crates/zenmon-tui/src/lib.rs`, after the existing imports (lines 1-13), add:
...
- Line 139 (inside `ConnectResult::Connected`): delete the line `app.nodes = zenmon_core::registry::list_nodes(&s).await.unwrap_or_default();`. The admin polling task already populates `app.admin_nodes` on its 2-second cadence.
...
Run: `grep -n "list_nodes" crates/zenmon-tui/src/lib.rs`
Expected: no output.

- [ ] **Step 6: Build**

Run: `cargo check -p zenmon-tui`
Expected: FAIL only on `crates/zenmon-tui/src/views/nodes.rs` (which still uses `node.kind.as_str()` patterns that will still work, plus potentially stale field reads). If `lib.rs` itself errors, stop and fix.
...
**Files:**
- Modify: `crates/zenmon-tui/src/views/nodes.rs` (full replacement)

- [ ] **Step 1: Replace the file contents**

Replace `crates/zenmon-tui/src/views/nodes.rs` with:
...
```bash
git add crates/zenmon-tui/src/views/nodes.rs
