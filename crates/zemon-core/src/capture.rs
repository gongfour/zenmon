...
fn b64_decode(s: &str, field: &str) -> Result<Vec<u8>, ZemonError> {
    base64::engine::general_purpose::STANDARD
        .decode(s)
        .map_err(|e| ZemonError::invalid_input(format!("invalid base64 in {}: {}", field, e)))
}
...
impl CaptureRecord {
    ...
    pub fn parse_line(line: &str, line_no: usize) -> Result<Self, ZemonError> {
    ...
    pub fn payload_bytes(&self) -> Result<Vec<u8>, ZemonError> {
        b64_decode(&self.payload_base64, "payload")
    }

    /// Decode the original attachment bytes, if any.
    pub fn attachment_bytes(&self) -> Result<Option<Vec<u8>>, ZemonError> {
