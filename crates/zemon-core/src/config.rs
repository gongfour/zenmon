...
impl ConfigEnvironment {
    pub fn from_process() -> Self {
        Self {
            endpoint: std::env::var("ZEMON_ENDPOINT").ok(),
            mode: std::env::var("ZEMON_MODE").ok(),
            namespace: std::env::var("ZEMON_NAMESPACE").ok(),
            config_file: std::env::var_os("ZEMON_CONFIG").map(PathBuf::from),
            scout_port: std::env::var("ZEMON_SCOUT_PORT").ok(),
            connect_timeout: std::env::var("ZEMON_CONNECT_TIMEOUT").ok(),
        }
    }
}
...
pub fn resolve_config_with_env(
    ...
    if let Some(value) = env.scout_port {
        let value = parse_scout_port(&value, "ZEMON_SCOUT_PORT")?;
    ...
    // connect_timeout: default (unset) -> env (ZEMON_CONNECT_TIMEOUT) -> cli.
    ...
    if let Some(value) = env.connect_timeout {
        ...
        let validated = validate_connect_timeout(parsed)
            .map_err(|msg| eyre!("invalid ZEMON_CONNECT_TIMEOUT '{}': {}", value, msg))?;
...
mod tests {
    ...
    fn config_file(contents: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("zemon-config-{nonce}.json5"));
