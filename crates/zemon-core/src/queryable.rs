...
pub fn resolve_reply_key(key_expr: &str, reply_key: Option<&str>) -> Result<String, ZemonError> {
    let ke = keyexpr::new(key_expr)
        .map_err(|e| ZemonError::invalid_input(format!("invalid key expression '{}': {}", key_expr, e)))?;

    match reply_key {
        Some(rk) => {
            let rke = keyexpr::new(rk).map_err(|e| {
                ZemonError::invalid_input(format!("invalid --reply-key '{}': {}", rk, e))
            })?;
            if rke.is_wild() {
        ...
        None => {
            if ke.is_wild() {
                Err(ZemonError::invalid_input(format!(
