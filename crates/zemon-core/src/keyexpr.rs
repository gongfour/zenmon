...
pub fn compare(a: &str, b: &str) -> Result<KeyExprRelation, ZemonError> {
    let ka = parse(a)?;
    let kb = parse(b)?;
...
fn parse(s: &str) -> Result<&keyexpr, ZemonError> {
    keyexpr::new(s)
        .map_err(|e| ZemonError::invalid_input(format!("invalid key expression '{}': {}", s, e)))
}
