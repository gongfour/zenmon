//! Integration tests for `ZEMON_CONNECT_TIMEOUT` validation.
//!
...
/// Run `zemon --json info` with the given `ZEMON_CONNECT_TIMEOUT`. `info` fails
...
fn run_with_timeout(value: &str) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_zemon"))
        .env("ZEMON_CONNECT_TIMEOUT", value)
        .args(["--json", "info"])
        .output()
